...
这部分记录一些和 LLM 共同编码时积累的经验，以及不同阶段可用的 prompts

### 归纳编码

- 最好在让 LLM 进行分析前自己快速浏览一遍数据，并带着问题与LLM交流。比如：
``` txt
本段数据的访谈问题是“如何描述该游戏的休闲体验”，你能从30个被访者的话里总结中几种不同性质的定义？
```
- 在每轮新数据中的编码前都询问编码复用的情况。比如：
```txt
之前已经建立的编码中，是否有可以在本轮数据中复用的？
```
- 对某个引文片段多重含义的加强解释。例如：
```txt
{某一个玩法}在玩家A处是正面评价，在玩家B处是负面评价。对于这个玩法的编码取决于你是否有其他编码作为锚点。一个语言对象的多种解释反映了人群偏好多样性。什么样的人在什么时候会感觉{玩法}是机械重复，或休闲放松。如果有多个变量锚定这个{玩法}，这是我们未来分析的重要信息。对于一个引文段来说，你看到什么就是什么，重要的是，能否识别出与{玩法}伴随性出现的其他变量（编码），以及编码在不同语句段中标注的含义是否一致。同样的理解也用于{玩法B}问题。

你体会下这个原则，可以加入到你的编码经验中。
```
- 编码目的是为了概念分析，因此编码名称要通俗易懂，易于交流，避免使用奇怪的自己造词。如果有相应的学术性规范用语，优先使用学术规范表达。比如：

```txt
学术性概念表达：心流、成就感、挑战、在场/社交在场..
通俗表达：收获感（不要使用“多巴胺奖励”）
```
...
- 聚焦主题。例如：
```txt
我们现在聚焦在{关注主题}的编码，请你搜索整个 raw codebook，看看有哪些编码与{关注主题}的相关、从哪些方面相关、为何相关。
1. 每一轮编码操作都要重头遍历所有编码
2. 第一轮从raw codebook的编码维度进行汇总，把所有与聚焦主题相关的编码进行汇总，并在{关注主题}的这个大主题下重新归纳概念。概念接近的编码检查“编码定义”，确定概念相同后可合并。注意合并后新编码必须保留所有并入编码的全部引文，不得删减。
3. 第二轮从raw codebook的引文中遍历，如果发现与主题相关的也进行提取。优先放入已有编码，如果概念与已有编码不兼容，再建立新编码
4. 小于3个引文的编码尽可能归人其他编码。如果无法归入则删除
5. 即使某些编码在前一主题里已被复用，仍然需要识别。每个旧编码可能包含多层含义。
